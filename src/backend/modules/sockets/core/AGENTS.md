...
- Path: `packages/apps/socket/src/core`
...
- Functions (all):
  - `parseTokenFromAuthHeader` (L3-L3, arrow)
  - `parseAccessTokenPayload` (L10-L10, arrow)
  - `parseTokenFromHandshake` (L32-L32, arrow)
- Exports:
  - `parseTokenFromAuthHeader` (L3)
  - `parseAccessTokenPayload` (L10)
  - `parseTokenFromHandshake` (L32)
- Key snippets and use-case mapping:
  - `L3-L3`: Implements `parseTokenFromAuthHeader` for this module use case.
  - `L10-L10`: Implements `parseAccessTokenPayload` for this module use case.
  - `L32-L32`: Implements `parseTokenFromHandshake` for this module use case.
### `build-session.socket.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
- Output shape: Typed return values, thrown errors, and exported contracts declared in this file.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `assertExistingUser` (L18-L18, arrow)
  - `buildSocketSession` (L24-L24, arrow)
- Exports:
  - `buildSocketSession` (L24)
- Key snippets and use-case mapping:
  - `L18-L18`: Implements `assertExistingUser` for this module use case.
  - `L24-L24`: Implements `buildSocketSession` for this module use case.
### `context.socket.ts`
- Purpose: Defines module behavior owned by this usage folder.
- Owning use cases: Runtime and application flows that import this file through package boundaries.
- Input shape: Typed arguments and imported contracts declared in this file signatures.
...
- Logic gates summary:
  ...
  - MCP or GraphQL proxy boundaries must avoid duplicated domain validation.
- Functions (all):
  - `emitLog` (L13-L13, arrow)
- Exports:
  - None
- Key snippets and use-case mapping:
  - `L13-L13`: Implements `emitLog` for this module use case.
